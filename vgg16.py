...
def build_model():
    ...
    x = tf.keras.layers.Dense(4096, activation='softmax')(x) #node개수 만큼 10 은 class Number 뒤에는 무조건 softmax 10개를 통해서 나오다 softmax를 통해서 하나만 나온다.
...
'''   
    ...
    input = tf.keras.Input(shape=(28, 28, 1), name='img') # 입력은 28/28/1이다.
    ...
    output = tf.keras.layers.Dense(10, activation='softmax')(x) #node개수 만큼 10 은 class 개수 뒤에는 무조건 softmax 10개를 통해서 나오다 softmax를 통해서 하나만 나온다.

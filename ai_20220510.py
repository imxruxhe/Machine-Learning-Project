...
def build_model():
    ...
    x = tf.keras.layers.Conv2D(
        filters=4, kernel_size=(3,3),strides=(1, 1),  padding='Same',activation='relu', use_bias=False)(input)
    # Conv2D - ReLu - MaxPool2D 가 이렇게 3개가 한 쌍이다.
    x = tf.keras.layers.MaxPool2D(pool_size=(2, 2),strides=(2,2), padding='Same')(x) # (28,28,4) stride(14,14,4) 
    
    x = tf.keras.layers.Conv2D(
    filters=8, kernel_size=(3,3),strides=(1, 1),  padding='Same',activation='relu', use_bias=False)(x) # (H,w,C) (14,14,8)
    x = tf.keras.layers.MaxPool2D(pool_size=(2, 2),strides=(2,2), padding='Same')(x) # (7,7,8) 
    
    x = tf.keras.layers.Conv2D(
    filters=16, kernel_size=(3,3),strides=(1, 1),  padding='Same',activation='relu', use_bias=False)(x) # (H,w,C) (7,7,16)
    ...
    output = tf.keras.layers.Dense(10, activation='softmax')(x) #node개수 만큼 10 은 class Number 뒤에는 무조건 softmax 10개를 통해서 나오다 softmax를 통해서 하나만 나온다.

...
def download_custom_dataset(key_word,saved_dir,number):  #인터넷 상애서 search 할 키워드,  saved 할 dir , 몇 장 찾을 것인가
...
def build_model():
    ...
    input = tf.keras.Input(shape=(64, 64, 3), name='img') # 입력은 28/28/1이다.
...
if __name__=='__main__':
    ...
    '''
    #datasets 한번만 다운하면 끝나는 것이라 끝남
    number=100
    classes = ['cat','dog'] 
    root_dir = './datasets'   # root_dir은 현재 datasets 아래에
    
    #downloadmake_custom_dataset(key_word,saved_dir,number)
    
    #download_custom_dataset('cat','./datasets/cat',number)
    #download_custom_dataset('cat','./datasets/dog',number)
    
    for i in range(len(classes)):   # 데이터의 개수가 100개 이거나 크면 이렇게 반복적으로 설정해주어야 한다.
